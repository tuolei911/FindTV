...
namespace BaseClasses
{
    public class HttpFunction
    {
        ...
        public static string Post(string url, Dictionary<string, string> data, Encoding encoding = null)
        {
            try
            {
                ...
                using (var client = new HttpClient(handler))
                {
                    ...
                    if (!response.IsSuccessStatusCode)
                    {
                      //  Log.ErrorFormat("HttpPost URL:{0},StatusCode", url, response.StatusCode.ToString());
                        return string.Empty;
                    }
                    response.Content.Headers.ContentType.CharSet = encoding.BodyName;
                    return response.Content.ReadAsStringAsync().Result;
        ...
        public static string PostJson(string url, string json, Encoding encoding = null)
        {
            try
            {
                ...
                using (var client = new HttpClient(handler))
                {
                    ...
                    if (!response.IsSuccessStatusCode)
                    {
                       // Log.ErrorFormat("HttpPost URL:{0},StatusCode{1}", url, response.StatusCode);
                        try
                        {
                            if (response.Content != null)
                            {
        ...
        public static string PostJsonGZip(string url, string json, Encoding encoding = null)
        {
            try
            {
                ...
                using (var client = new HttpClient(handler))
                {
                    ...
                    var g = BaseClasses.GZipHelper.Compress(array);
        ...
        public static string UploadFile(string filePath, Dictionary<string, string> param = null)
        {
            var url = ConfigurationHelper.GetAppSetting<string>("UploadFile");
